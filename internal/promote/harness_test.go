...
func (f *fakeFS) addFile(path string, size int64) *fakeFile {
	return f.put(path, &fakeFile{
		size:   size,
		mode:   0o640,
...
func newHarness(t *testing.T) *harness {
	...
	h := &harness{
		...
		copier: &mock.CopierMock{CopyFunc: func(_ context.Context, src, dst string) (int64, error) {
			rec.add("copy", src+" -> "+dst)
			ffs.addFile(dst, sourceSize/2)

			return sourceSize / 2, nil
		}},
...
// requireAdjacent asserts that b is the very next recorded call after the last a.
func requireAdjacent(t *testing.T, calls []string, a, b string) {
	t.Helper()
	...
	require.NotEqual(t, -1, last, "call %q never happened in %v", a, calls)
	require.Less(t, last, len(calls)-1, "nothing recorded after %q", a)
	require.Equal(t, b, calls[last+1], "expected %q immediately after %q, got %v", b, a, calls)
}
