  ...
  return (
    <div className="space-y-6 p-8">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">What gets transcoded, and to what.</p>
      </header>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-sky-800 bg-sky-950/50 p-4">
        ...
        <div className="text-sm text-sky-100">
          <p className="font-semibold">
            This is compiled into the binary. There is nothing to edit.
          </p>
          <p className="mt-1 text-xs text-sky-200">
            No setting anywhere in Codarr changes any value on this page. Changing the policy means
            changing Go constants and rebuilding, which changes the policy hash below and makes
            every already-tagged file eligible for a re-check again.
          </p>
      ...
      <Panel title="Video" icon="video">
        <div className="space-y-5">
          <dl className="grid gap-4 sm:grid-cols-2">
            ...
            <KeyValue label="Chroma subsampling">
              {policy.video.copy_rule.chroma_subsampling}
            </KeyValue>
            ...
            <KeyValue label="Progressive only">
              {yesNo(policy.video.copy_rule.progressive_only)}
            </KeyValue>
            <KeyValue label="Unknown scan counts as progressive">
              {yesNo(policy.video.copy_rule.unknown_scan_is_progressive)}
            </KeyValue>
            <KeyValue label="Drop attached pictures">
              {yesNo(policy.video.drop_attached_pictures)}
            </KeyValue>
          </dl>
          ...
          <div className="border-t border-slate-800 pt-4">
            ...
            <p className="mb-2 text-xs text-slate-400">
              A stream that fails only the level test is still copied. The flag is rewritten
              in-stream during the container rebuild.
            </p>
      ...
      <Panel title="Audio" icon="audio">
        <div className="space-y-5">
            ...
            <ul className="space-y-1 text-sm text-slate-300">
              {policy.audio.encode_targets.map((target, index) => (
                <li key={index}>
                  ...
                  {target.downmix_channels
                    ? `, downmixed to ${target.downmix_channels} channels`
                    : ''}
                </li>
              ))}
            </ul>
          ...
          <dl className="grid gap-4 border-t border-slate-800 pt-4 sm:grid-cols-2">
            <KeyValue label="MP4 multichannel codec">
              {policy.audio.mp4_multichannel_codec}
            </KeyValue>
            ...
            <KeyValue label="Never zero audio streams">
              {yesNo(policy.audio.never_zero_audio_streams)}
            </KeyValue>
          </dl>
        </div>
      </Panel>
      ...
      <Panel title="Subtitles" icon="subtitle">
        <dl className="space-y-3">
          ...
          <KeyValue label="Targets">
            {policy.subtitles.targets
              .map((target) => `${target.container} to ${target.codec}`)
              .join(', ')}
          </KeyValue>
          <KeyValue label="Drop attachments">{yesNo(policy.subtitles.drop_attachments)}</KeyValue>
          <KeyValue label="Keep all languages">
            {yesNo(policy.subtitles.keep_all_languages)}
          </KeyValue>
          ...
          <div className="border-t border-slate-800 pt-4">
            ...
            <dl className="grid gap-4 sm:grid-cols-4">
              <KeyValue label="Segments">
                {policy.bitrate.sample_probe.segments} x{' '}
                {policy.bitrate.sample_probe.segment_seconds}s
              </KeyValue>
              <KeyValue label="Skip head/tail">
                {policy.bitrate.sample_probe.skip_head_pct}% /{' '}
                {policy.bitrate.sample_probe.skip_tail_pct}%
              </KeyValue>
              ...
              <KeyValue label="Hardware correction">
                {policy.bitrate.sample_probe.hardware_correction}
              </KeyValue>
      ...
      <Panel title="Space reclaim sweep" icon="disk">
        <dl className="grid gap-4 sm:grid-cols-4">
          ...
          <KeyValue label="Minimum video bitrate">
            {policy.space_sweep.min_video_bitrate_kbps} kbps
          </KeyValue>
          <KeyValue label="Minimum projected saving">
            {policy.space_sweep.min_projected_saving_pct}%
          </KeyValue>
        </dl>
      </Panel>
      ...
      <Panel title="Exclusions" icon="ban">
        <dl className="space-y-3">
          ...
          <KeyValue label="Stability guard">
            {policy.exclusions.stability_guard_seconds} seconds
          </KeyValue>
        </dl>
      </Panel>
